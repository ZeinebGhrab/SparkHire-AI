...
- Services ASR/TTS/Avatar chargés une seule fois au démarrage (instances globales)
  pour éviter le timeout WebSocket pendant le chargement des modèles.
- Vérification de connexion (_check_connected) avant chaque étape majeure.
- Propagation correcte de WebSocketDisconnect pour arrêt propre du handler.
- _send_error vérifie la connexion avant d'envoyer.
"""
...
# Initialisées une seule fois dans main.py au démarrage du serveur.
# Cela évite de recharger Vosk (~7s) et Coqui TTS (~30-60s)
# à chaque nouvelle connexion WebSocket.
# ============================================================
_asr_service = None
_tts_service = None
...
class InterviewHandler:
    ...
    def __init__(self, session_id: str, websocket: WebSocket):
        ...
        # Utiliser les instances globales pré-chargées
        ...
        if self.asr_service is None:
            logger.warning("ASR non disponible - les réponses ne seront pas transcrites")
    ...
    def _check_connected(self):
        """
        Vérifie que le client WebSocket est encore enregistré dans le manager.
        Lève WebSocketDisconnect si ce n'est plus le cas,
        ce qui interrompt proprement la boucle principale du handler.
        """
    ...
    async def handle(self):
        ...
        try:
            ...
            # Laisser le temps au WebSocket de se stabiliser
    ...
    async def _wait_for_answer(self):
        ...
        # Vérification préalable avant d'entrer dans la boucle bloquante
        self._check_connected()
    ...
    async def _process_answer(self, start_time: Optional[datetime]):
        ...
        # Notification au client (si encore connecté)
        if manager.is_connected(self.session_id):
    ...
    async def _complete_interview(self):
        """Terminer l'entretien et envoyer le message de clôture"""
    ...
    async def _send_error(self, message: str, error_type: str = "GENERAL_ERROR"):
        ...
        if not manager.is_connected(self.session_id):
            logger.debug(
                f"Erreur non envoyée (client déconnecté): {error_type} - {message}"
            )
            return
        ...
        try:
            ...
            pass  # Client déjà parti, on ignore
    ...
    def _synthesize_b64(self, text: str, language: str) -> Optional[str]:
        """
        Synthétiser du texte en audio et retourner le résultat encodé en base64.
        Retourne None si le TTS est indisponible ou si la synthèse échoue.
        """
        ...
        try:
            audio_data = self.tts_service.synthesize(text, language=language, use_cache=True)

            if audio_data and len(audio_data) > 0:
                b64 = base64.b64encode(audio_data).decode("utf-8")
                logger.info(f"Audio synthétisé: {len(audio_data)} bytes → {len(b64)} chars base64")
                return b64
            else:
